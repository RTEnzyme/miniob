...
struct SubtractOperator
{
  ...
  { 
    return _mm256_sub_ps(left, right); // 浮点数减法
  }

  static inline __m256i operation(__m256i left, __m256i right) 
  {
    return _mm256_sub_epi32(left, right); // 整数减法
  }
#endif
};
...
// your code here
    ...
    static inline __m256 operation(__m256 left, __m256 right)
    {
        return _mm256_mul_ps(left, right); // 浮点数乘法
    }

    static inline __m256i operation(__m256i left, __m256i right)
    {
        return _mm256_mullo_epi32(left, right); // 整数乘法
    }
#endif
};
